...
% SOL				    LIS DE REZ 						OPERATIE
% vid 				(x*y)*z = u * u ^(-1)
% vid 				*(*(x, y), z) = *(u, ^(-1)(u))			DESC
% vid 				(x*y) = u, z = u^(-1)					REZOLVA 
% u=x*y 			z = (x*y)^(-1)							REZOLVA
% u = x*y, z = (x*y)^(-1)			vid 					FINAL

% unificator: {x*y/u, (x*y)^(-1)/z}
...
pathaux(X, Y, _) :- connected(X, Y).
pathaux(X, Y, L1) :-
	not(vis(Z, L1)), connected(X, Z), pathaux(Z, Y, [Z | L1]).
